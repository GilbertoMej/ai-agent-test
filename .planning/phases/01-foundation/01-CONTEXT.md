...
- **D-22:** **Auth shape (BCK-02)** = env-var bearer token for the operator, no login screen in Phase 1. The `WORKER_SHARED_SECRET` env var is the only credential; Next.js sends it as a Bearer header on every `/api/chat`, `/api/approve`, `/api/health` call. Single-user demo; no UI auth needed. Rationale: Phase 1 is a local-only playground (D-05) — the operator is the only human touching the UI; a login screen adds friction for zero isolation benefit. — **Reversibility:** one-way — adding real auth later means a login screen + token rotation + session lifecycle.
